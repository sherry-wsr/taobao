...
- **A**ction：
  ...
  2. **建立 3 层指标体系**（北极星 MPAU + 活动/转化/质量指标），确保每个分析都有锚点
  3. 四步漏斗分析，定位"浏览→收藏"流失 97.90% 为最大瓶颈（按事件次数）
  4. **假设驱动归因**：先列假设再用数据验证——H1 "window shopping" 被接受，H2 "scattered browsing" 被反驳（drop-off 用户其实更聚焦），得出 "focused but failing" 新模型
  5. **异动归因方法论**：4 步框架（定位→拆解→假设→验证），自动识别出 12 月 12 日双 12 异常（z-score +2.25）
  6. RFM 8 类分群 + Cohort 留存分析（D1 是关键流失点）
...
3. **11.14% 用户从未购买**：归因为"focused but failing"——他们更聚焦而非分散
4. **RFM 8 类分布不均**：最大群体是 Potential loyalists（23.8%），核心价值是 Champions（14.8%）
5. **黄金时段是 20-22 点**：浏览与购买同时达峰
6. **方法论发现**：双 12（12月12日）购买率异常飙升至 2.38%（z-score +2.25）——验证了 4 步异动归因框架的有效性

## 三大分析能力展示

本项目针对数据分析师面试最看重的 3 项能力分别有专门设计：

### 1. 指标体系搭建

- **北极星指标**：MPAU（月度活跃购买用户数）
- **3 层金字塔**：活动（量）→ 转化（率）→ 质量（质）
- **跨 notebook 一致**：每个 notebook 回答特定层的指标问题（详见 Notebook 05 开头）

### 2. 异动归因

- **4 步方法论**：定位（z-score 异常检测）→ 拆解（维度分解）→ 假设（外部/内部/数据）→ 验证
- **真实案例**：12 月 12 日双 12 自动被识别为唯一异常日
- **代码实现**：`scripts/utils.py` + Notebook 05 的滚动基线 + z-score 检测

### 3. 数据思维

- **假设驱动分析**：Notebook 02 的 H1（接受）和 H2（反驳）展示了 falsifiable 思维
- **数据质量决策**：Notebook 01 不简单删除异常，而是逐个评估
- **方法选择取舍**：漏斗用事件次数 vs 独立用户、RFM 用品类多样性代替金额——每个选择都在 README "关键技术决策" 解释
...
9. **归因采用假设驱动而非探索式**：先列假设再验证，H1 接受、H2 反驳——后者反而得出更有价值的"focused but failing"模型。
10. **异动检测用滚动基线 + z-score**：避免单日绝对比较的噪声，7 天滚动均值更稳健。
...
| Notebook | 一句话说明 |
|---------|----------|
| 01 - 数据探索 | 数据规模、字段类型、4 种行为分布、用户活跃度（含质量决策）|
| 02 - 漏斗分析 | 4 步漏斗 + 分层（时段/品类）+ **假设驱动归因**（核心）|
| 03 - RFM 分群 | 8 类用户群体 + 差异化运营策略 |
| 04 - Cohort 留存 | 30 天留存热力图 + 整体留存曲线 |
| 05 - 总结 | **指标体系** + **异动归因方法论** + 业务洞察 + 量化影响 + 数据局限 |
